...
class BeneficiaryLoginView(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({
            'token': token.key,  # Simple token (instead of JWT)
            'user_id': BeneficiaryUserRegistration.objects.get(benficiary_user_id=user.pk).pk,
            'username': user.username,
        }, status=status.HTTP_200_OK)
